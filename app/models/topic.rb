...
class Topic < ActiveRecord::Base
  ...
  # 软删除（deleted_at、deleted_by_id字段）
  include Trashable
  # 检索字段
  include Searchable
  # 限制编辑删除时效
  ...
  # 使用自定义验证类，验证类在lib/validators下定义
  # UniqueAmongValidator的unless选项依赖于ActiveRecord::Validations::UniquenessValidator的功能，
  # :if, :unless选项由ActiveSupport::Callbacks模块支持
  # :allow_nil, :allow_blank选项由ActiveModel::EachValidator模块支持
  # :case_sensitive选项由ActiveRecord::Validations::UniquenessValidator模块支持
  # :message选项由ActiveModel::Errors模块支持
  # :collection选项由UniqueAmongValidator模块自己使用
